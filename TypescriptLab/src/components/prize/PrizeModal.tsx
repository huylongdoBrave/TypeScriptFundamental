...
interface PrizeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (prizeData: NewPrizeData | Prize) => void;
  prizeToEdit?: Prize | null; // Prop mới để truyền giải thưởng cần sửa
}

function PrizeModal({ isOpen, onClose, onSave, prizeToEdit }: PrizeModalProps) {
  const isEditMode = !!prizeToEdit;
  ...
  useEffect(() => {
    if (isEditMode && prizeToEdit) {
      setName(prizeToEdit.name);
      setQuantity(prizeToEdit.quantity);
      setType(prizeToEdit.type);
      setStatus(prizeToEdit.status);
    }
  }, [prizeToEdit, isEditMode, isOpen]); // Chạy lại khi modal mở hoặc prize thay đổi
...
//   };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const prizeData = { name, quantity, type, status };
    if (isEditMode && prizeToEdit) {
      onSave({ ...prizeData, id: prizeToEdit.id });
    } else {
      onSave(prizeData);
    }
    onClose();
  ...
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={handleClose}>
      ...
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-2xl font-bold mb-6 text-gray-800">{isEditMode ? 'Sửa Giải Thưởng' : 'Thêm Giải Thưởng Mới'}</h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-4">
            <button type="button" onClick={handleClose} className="py-2 px-4 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">Hủy</button>
